...
download_release() {
  ...
  # exemple 4 https://github.com/scaleway/scaleway-cli/releases/download/v2.5.1/scaleway-cli_2.5.1_darwin_amd64
  ...
  url4="$GH_REPO/releases/download/v${version}/scaleway-cli_${version_slug_2}_${target_os}_amd64${target_ext}"
  ...
  curl -fs "${curl_opts[@]}" -o "$filename" -C - "$url1" ||
    ...
    curl -fs "${curl_opts[@]}" -o "$filename" -C - "$url4" ||
    fail "Could not download $url1 or $url2 or $url3 or $url4"
}
